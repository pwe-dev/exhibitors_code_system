...
while (have_posts()):
	...
	function show_taxes($tax){
		if($tax == 'email'){
			...
			$WebSite = strip_tags(get_the_term_list( $post->ID, $tax));
			if (strpos($WebSite, "http")!==false){
			return '<div style="text-transform: lowercase;"><a style="color: #222222; text-decoration: underline;" href="'.$WebSite .'">' . strip_tags(get_the_term_list( $post->ID, $tax)) .'</a></div><br>';
			} else {
				return '<div style="text-transform: lowercase;"><a style="color: #222222; text-decoration: underline;" href="https://'.$WebSite .'">' . strip_tags(get_the_term_list( $post->ID, $tax)) .'</a></div><br>';
			}
		}		
		else{
	...
	if ($show_title) {
		...
		$title_content .= '<div class="post-info">' . 'Hala / Hall: '. show_taxes('hall') . 'Numer Stoiska / Stand Number: ' .show_taxes('stoisko') . 'Email : '. show_taxes('email') . 'Numer telefonu / Phone Number: '. show_taxes('telefon') . 'Strona Internetowa / Website: ' .show_taxes('strona') .'</div>';
		$title_content .= '</div>';
		$title_content .= apply_filters( 'uncode_after_body_title', '' );
	}

	/** Build content **/
	$the_content = '<strong style="color: #222222 !important">Opis Wystawcy / Comapny Description </strong><br>';
